...
class HomeLatestImageCell: UITableViewCell,Reuseable {
    ...
    var popularLayout: HomePopbarLayout! {
        didSet {
            // 设置图片位置
            linkImageView.extSetImage(NSURL(string: popularLayout.model.image))
            linkImageView.frame = popularLayout.kHomeCellPicRect
            
            // 设置分类和时间
            let dateAttributeText = NSMutableAttributedString(string: "\(popularLayout.model.category) | \(NSDate.getCommonExpresionOfDate(popularLayout.model.pubDate))")

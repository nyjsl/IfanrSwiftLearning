...
struct HomePopbarLayout: Initable{
    ...
    private func calculateDateSize() -> CGSize{
        let dateStr = "\(model.category) | \(NSDate.getDate(model.pubDate)))"
        let toolbarAttribute = [NSFontAttributeName:UIFont.customFont_FZLTXIHJW(fontSize: 12)]
        return (dateStr as NSString).boundingRectWithSize(CGSize(width: 200, height: kHomeCellToolBarHeight), options: .UsesLineFragmentOrigin, attributes: toolbarAttribute, context: nil).size
